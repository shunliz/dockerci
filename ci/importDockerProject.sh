...
curl --request POST --user "${GERRIT_ADMIN_UID}:${GERRIT_ADMIN_PWD}" --data "${DEMO_DOCKER_CONFIG_XML}" --header "Content-Type: application/xml;charset=UTF-8" ${JENKINS_WEBURL}/createItem?name=demo-docker
